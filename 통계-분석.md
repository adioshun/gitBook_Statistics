...
* eg : 평균검정(t-test), ANOVA, ANCOVA, MANOVA
    * One sample T test : 하나의 집단의 평균이 얼마인지를 검정(eg. 베스킨 라빈스 A파인트컵 30개)
    * Independent Sample T test : 두 집단간의 차이가 있는지 비교
    * Paired Sample T test : 하나의 집단이지만 사건 전/후로 나누어서 비교 (eg. 다이어트 효과)
    * One way ANOVA : 한 개의 집단 구분변수
    * Two way ANOVA : 두 개의 집단 구분 변수를 통시에
    * Repeated Measures ANOVA : 집단이 세 개이면서 반복 측정
